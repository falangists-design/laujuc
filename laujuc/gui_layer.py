...
class GlobalHotkeyManager:
    def __init__(self, app: QtWidgets.QApplication, callback: callable) -> None:
        self.app = app
        self.callback = callback
        self.hotkey_id = 1
        self._filter: QtCore.QAbstractNativeEventFilter | None = None

    def register(self, key_name: str) -> None:
        ...
        if ctypes.windll.user32.RegisterHotKey(None, self.hotkey_id, 0x0000, vk):
    ...
    def _make_filter(self) -> QtCore.QAbstractNativeEventFilter:
        import ctypes
        ...
        class Filter(QtCore.QAbstractNativeEventFilter):
            def nativeEventFilter(self, event_type: bytes, message: int) -> tuple[bool, int]:
                ...
                if msg.message == 0x0312 and msg.wParam == manager.hotkey_id:
                    manager.callback()
                    return True, 1
                return False, 0
    ...
    @staticmethod
    def _key_to_vk(key_name: str) -> int | None:
        mapping = {
            "Insert": 0x2D,
            "insert": 0x2D,
            "INS": 0x2D,
...
class LaujucWindow(QtWidgets.QWidget):
    ...
    def _setup_shortcuts(self) -> None:
        ...
        if app and platform.system().lower() == "windows":
            self._hotkey_manager = GlobalHotkeyManager(app, self._toggle_visibility)
            self._hotkey_manager.register(self.settings.hotkey_toggle)
